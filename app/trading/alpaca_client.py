...
def get_account():
    ...
    log.debug(
        ...
        extra={
            ...
            "daytrading_buying_power": str(getattr(account, "daytrading_buying_power", "N/A")),
...
def get_daytrading_buying_power() -> float:
    """Return the current Day Trading Buying Power (DTBP). Returns 0.0 on error."""
    try:
        account = get_account()
        dtbp = getattr(account, "daytrading_buying_power", None)
        return float(dtbp) if dtbp is not None else float(account.buying_power)
    except Exception as exc:
        log.warning("Could not fetch DTBP: %s", exc)
        return 0.0
...
def get_portfolio_history(period: str, timeframe: str = "1D"):
    """Fetch portfolio equity history for a given period (e.g. '1W', '1M', '1A')."""
    return get_client().get_portfolio_history(
        GetPortfolioHistoryRequest(period=period, timeframe=timeframe)
    )


def get_market_calendar(start, end):
    """Return trading calendar entries between start and end dates."""
    return get_client().get_calendar(GetCalendarRequest(start=start, end=end))
