...
async def execute_action(payload: AlertPayload) -> dict:
    ...
    if action == TradingAction.BUY:
        order = _require_qty_then_order(ticker, OrderSide.BUY, qty, payload.price, payload.limit_price, payload.time_in_force, payload.extended_hours)
        result["orders"].append(_order_summary(order))

    elif action == TradingAction.SELL:
        order = _require_qty_then_order(ticker, OrderSide.SELL, qty, limit_price=payload.limit_price, time_in_force=payload.time_in_force, extended_hours=payload.extended_hours)
        result["orders"].append(_order_summary(order))
        ...
        long_order = _require_qty_then_order(ticker, OrderSide.BUY, qty, payload.price, payload.limit_price, payload.time_in_force, payload.extended_hours)
        ...
        if close_order:
            result["orders"].append(_order_summary(close_order))
        short_order = _require_qty_then_order(ticker, OrderSide.SELL, qty, limit_price=payload.limit_price, time_in_force=payload.time_in_force, extended_hours=payload.extended_hours)
    ...
    # Grouping Level 1 and Level 2 together since they use the same logic
        ...
        order = _kimi_add_leverage(ticker, math.floor(qty or 0), payload.price, payload.limit_price, payload.time_in_force, payload.extended_hours)
        if order:
            result["orders"].append(_order_summary(order))
        else:
...
def _kimi_add_leverage(ticker: str, qty: int, price: Optional[float] = None, limit_price: Optional[float] = None, time_in_force: str = "gtc", extended_hours: bool = False) -> Optional[Order]:
    """
    ...
    log.info(
        "Placing Kimi DD buy",
        extra={"ticker": ticker, "requested_qty": qty, "actual_qty": actual_qty, "limit_price": limit_price, "time_in_force": time_in_force, "extended_hours": extended_hours},
    )
    if limit_price:
        return ac.place_limit_order(ticker, OrderSide.BUY, actual_qty, limit_price, time_in_force, extended_hours)
...
def _require_qty_then_order(
    ...
    if limit_price:
        return ac.place_limit_order(ticker, side, qty, limit_price, time_in_force, extended_hours)
