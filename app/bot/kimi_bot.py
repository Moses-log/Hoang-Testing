"""
kimi_bot.py — Kimi No-Stop-Loss trading bot.

Runs as a Render Background Worker alongside the FastAPI webhook server.
Subscribes to live 1-min bars via Alpaca WebSocket, calculates Bollinger
Bands locally, and places orders using the shared alpaca_client module.
Discord notifications go through the shared notifications module.

Key env vars (all via app/config.py):
  BOT_SYMBOLS        — comma-separated e.g. "QQQ,SPY,NVDA"
  BOT_DTBP_USAGE     — fraction of day-trading buying power to use (0.5 = 50%)
  BOT_BB_LENGTH      — Bollinger Band period (default 20)
  BOT_STOP_LOSS_PCT  — stop % below entry (default 50)
  BOT_LEV_BAR_GAP    — min bars between entries (default 30)
"""
...
from app.notifications import notify, notify_trades_embed

log = logging.getLogger("KimiBot")

# =============================================================================
# CONFIGURATION — driven entirely by env vars via settings
# =============================================================================
...
# STARTUP CAPITAL
# =============================================================================
INITIAL_CAPITAL    = ac.get_daytrading_buying_power() * DTBP_USAGE
...
# HISTORICAL BARS
# =============================================================================
_hist = StockHistoricalDataClient(settings.alpaca_api_key, settings.alpaca_secret_key)

def fetch_bars(symbol: str, start, end) -> list:
    try:
        bars = _hist.get_stock_bars(StockBarsRequest(
            symbol_or_symbols=symbol,
            timeframe=TimeFrame.Minute,
            start=start, end=end,
            session="regular", feed="iex",
        )).df
        if bars.empty:
            return []
...
def preload_bars(symbol: str) -> None:
    ...
    if basis:
        log.info("[%s] ✅ %d bars | Last=$%.2f | BB[%.2f/%.2f/%.2f]",
                 symbol, len(bars), bars[-1]["close"], lower, basis, upper)
    else:
        log.warning("[%s] %d bars loaded — BB still warming up", symbol, len(bars))
...
def replay_yesterday(symbol: str) -> None:
    ...
    if not bars:
        log.warning("[%s] No yesterday bars", symbol)
        return
    ...
    for b in bars:
        ...
        if basis is not None:
            ...
            if not s["has_lev1"] and c <= lower and bar_gap_ok:
                qty = max(1, int(buying_power(s) * LEVERAGE_FACTOR / c))
    ...
    # Reconcile with live Alpaca position
    pos = ac.get_position(symbol)
    if s["has_lev1"] and pos is None:
    ...
    elif not s["has_lev1"] and pos is not None:
        qty   = float(pos.qty)
        ...
        s.update({"has_lev1": True, "lev1_entry_qty": int(qty),
                  "lev1_entry_price": price, "lev1_deployed": qty * price})
...
def sync_position(symbol: str, current_price: float) -> str | None:
    ...
    if not s["has_lev1"] and qty > 0:
        log.warning("[%s] 🔄 Manual entry | Qty=%d @ $%.2f", symbol, qty, avg_price)
        s.update({"has_lev1": True, "lev1_entry_qty": int(qty),
...
# =============================================================================
# SIGNAL LOGIC
# Returns an event dict so the async on_bar can send Discord notifications.
# =============================================================================
def process_bar(symbol: str, bar: dict) -> dict | None:
    s = states[symbol]
    s["bar_index"] += 1
    c = bar["close"]
    s["bars"].append(c)
    ...
    log.info("[%s] #%d C=%.2f Lo=%.2f Mid=%.2f Hi=%.2f Pos=%s BP=$%,.0f",
             symbol, s["bar_index"], c, lower, basis, upper, position_qty, buying_power(s))
    ...
    if not s["has_lev1"]:
        ...
        if near_lower and not s["alerted_lower"]:
            ...
            return {"type": "alert",
                    "message": f"⚠️ **{symbol}** approaching lower BB | ${c:.2f} → lower ${lower:.2f}"}
        if not near_lower:
        ...
        if near_mid and not s["alerted_mid"]:
            s["alerted_mid"] = True
            return {"type": "alert",
                    "message": f"🎯 **{symbol}** approaching middle BB — prepare to exit | ${c:.2f}"}
        if not near_mid:
            s["alerted_mid"] = False
        ...
        if near_upper and not s["alerted_upper"]:
            ...
            return {"type": "alert",
                    "message": f"🚀 **{symbol}** approaching upper BB | ${c:.2f}"}
        if not near_upper:
    ...
    # ── 1. Entry ──────────────────────────────────────────────────────────────
    if not s["has_lev1"] and position_qty == 0 and c <= lower and bar_gap_ok and in_entry_window():
        bp  = buying_power(s)
        qty = max(1, int(bp * LEVERAGE_FACTOR / c))
        try:
            ac.place_market_order(symbol, OrderSide.BUY, qty)
            ok = True
        except Exception as exc:
            log.error("[%s] Entry order failed: %s", symbol, exc)
            ok = False

        if not ok:
            real_dtbp = ac.get_daytrading_buying_power()
            real_qty  = int(real_dtbp / c)
            if real_qty > 0:
                try:
                    ac.place_market_order(symbol, OrderSide.BUY, real_qty)
                    ok  = True
                    qty = real_qty
                except Exception as exc:
                    log.error("[%s] Retry entry failed: %s", symbol, exc)
        ...
        if ok:
            ...
            return {"type": "entry", "symbol": symbol, "price": c, "qty": qty,
                    "capital": bp, "stop": stop_price}
        else:
            return {"type": "alert",
                    "message": f"🚫 **{symbol}** entry failed — insufficient buying power at ${c:.2f}"}
    ...
    if s["has_lev1"] and s["lev1_entry_price"] and c > exit_level and c > s["lev1_entry_price"] * 1.0003:
        ...
        entry   = s["lev1_entry_price"]
        s["session_pnl"] += pnl_usd; s["net_profit"] += pnl_usd; s["trade_count"] += 1
        s["lev1_deployed"] = 0.0
        s.update({"alerted_lower": False, "alerted_mid": False, "alerted_upper": False})
        try:
            ac.place_market_order(symbol, OrderSide.SELL, qty)
        except Exception as exc:
            log.error("[%s] Exit order failed: %s", symbol, exc)
        s.update({"has_lev1": False, "lev1_entry_qty": 0, "lev1_entry_price": None})
        save_state()
        log.info("⬇ [%s] EXIT $%.2f P/L=%.2f%% $%.2f Trade#%d",
                 symbol, c, pnl_pct, pnl_usd, s["trade_count"])
        return {"type": "exit", "symbol": symbol, "price": c, "qty": qty,
                "entry": entry, "pnl": pnl_usd, "pnl_pct": pnl_pct, "trade_n": s["trade_count"]}
    ...
    # ── 3. Stop Loss ──────────────────────────────────────────────────────────
        ...
        pnl_pct = (c - s["lev1_entry_price"]) / s["lev1_entry_price"] * 100
        entry   = s["lev1_entry_price"]
        s["session_pnl"] += pnl_usd; s["net_profit"] += pnl_usd
        s["lev1_deployed"] = 0.0
        s.update({"alerted_lower": False, "alerted_mid": False, "alerted_upper": False})
        try:
            ac.place_market_order(symbol, OrderSide.SELL, qty)
        except Exception as exc:
            log.error("[%s] Stop order failed: %s", symbol, exc)
        s.update({"has_lev1": False, "lev1_entry_qty": 0, "lev1_entry_price": None})
        save_state()
        log.warning("🛑 [%s] STOP $%.2f P/L=%.2f%% $%.2f", symbol, c, pnl_pct, pnl_usd)
        return {"type": "stop", "symbol": symbol, "price": c, "qty": qty,
                "entry": entry, "pnl": pnl_usd, "pnl_pct": pnl_pct}

    # ── Safety reset ──────────────────────────────────────────────────────────
    if ac.get_position(symbol) is None:
        s.update({"has_lev1": False, "lev1_entry_qty": 0, "lev1_entry_price": None,
                  "lev1_deployed": 0.0, "last_lev_bar": -999,
                  "alerted_lower": False, "alerted_mid": False, "alerted_upper": False})
        save_state()
...
# =============================================================================
# WEBSOCKET HANDLER — async, sends Discord after processing
# =============================================================================
...
if __name__ == "__main__":
    ...
    log.info("📡 Subscribed to %d symbols — listening for bars", len(SYMBOLS))
    stream.run()
